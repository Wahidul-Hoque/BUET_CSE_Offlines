...
bool test_size(Stack* stack) {
    ...
    int s=stack->size();
    return s==3 && stack->top() == 50; // Placeholder
}
...
bool test_multiple_push_pop(Stack* stack) {
    ...
    return stack->top()==90 && stack->size()==90 && g==91 ; // Placeholder
}
...
bool test_empty_stack_operations(Stack* stack) {
    ...
    arr=stack->pop();
    list=stack->top();
    return arr==-1 && list==-1; // Placeholder
}
...
bool test_array_resizing(Stack* stack) {
    ...
    if(stack->size()!=100)ans=false;
    for(int i=100;i>0;i--){
        if(i!=stack->pop())ans=false;
    }
    return ans && stack->size()==0 ; // Placeholder
}
...
bool test_stress(Stack* stack) {
    ...
    return first==9000 && sec==10000; // Placeholder
}
